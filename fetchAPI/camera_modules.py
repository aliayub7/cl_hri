...
class YoloDetector():
    ...
    def get_item_3d_coordinates(self,i_name=None, rgb_img=None,obj_box=None):
        if obj_box is not None:
            ...
            up_3d = self._2dto3d.pixelTo3DPoint(int(box[0] + box[2])//2 + w*.25,int(box[1] + box[3])//2 + h *.4)
...
class Get3Dcoordinates(object):
    ...
    def pixelTo3DPoint(self, u, v, cloud=None):
        ...
        #print ('len target_xyz_cam',len(target_xyz_cam))
        #print ('target_xyz_cam',target_xyz_cam)
        if len(target_xyz_cam)==0:
...
if __name__ == '__main__':
    ...
    """
    print (yd.class_names)
    print (yd.boxes)
    boxes = yd.boxes
    class_names = yd.class_names
    coord_3ds = []
    for i in range(0,len(boxes)):
        coord_3d,_ = yd.get_item_3d_coordinates(obj_box=boxes[i])
        coord_3ds.append(coord_3d)
    print (coord_3ds)
    """
    ...
    import ctypes
    import struct
    cloud = yd._2dto3d.cloud
    print (cloud.fields)
    gen = pc2.read_points(cloud,skip_nans=True)
    gen = list(gen)

    xyz = np.empty((len(gen), 3))#np.array([[0,0,0]])
    rgb = np.empty((len(gen), 3))#np.array([[0,0,0]])
    #xyz = np.array([[0,0,0]])
    #rgb = np.array([[0,0,0]])

    for id in range (len(gen)):
        x = gen[id]
        test = x[3]
        ...
        xyz[id] = np.array([[x[0],x[1],x[2]]])
        rgb[id] = np.array([[r,g,b]])
    ...
    while 1:
        ...
        #    continue
        try:
            cv2.imshow("RGB Camera", yd.getDetectionImage())
        except:
            continue
